...
def test_usages_with_frt_list():
    ...
    if not all(
        [os.getenv("ENERBIT_API_BASE_URL"), os.getenv("ENERBIT_API_USERNAME"), os.getenv("ENERBIT_API_PASSWORD")]
    ):
        return  # Skip test if env vars not set

    command = USAGES_ARGLIST + ["fetch", "--since=2023-01-01", "--until=2023-01-02", "Frt00000"]
