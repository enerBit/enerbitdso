...
class TestMyLibrary(unittest.IsolatedAsyncioTestCase):
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_all_usage_records(self, mock_get_auth_token):
        ...
        with patch(
            ...
            usages = await ebconnector.fetch_schedule_usage_records_large_interval(
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_part_usage_records(self, mock_get_auth_token):
        ...
        for usage in usages:
            self.assertIn(usage, mocked_usages, "The usage is not in mocked usages list")

    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_empty_usage_records(self, mock_get_auth_token):
        today = dt.datetime.now()
        since_month = today - dt.timedelta(weeks=WEEKS_TO_TEST)
        until_month = today
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_all_schedule_records(self, mock_get_auth_token):
        today = dt.datetime.now()
        since_month = today - dt.timedelta(weeks=WEEKS_TO_TEST)
        until_month = today
        frontier = "Frt" + "".join(random.choices("0123456789", k=5))
        ...
        with patch(
            ...
            schedules = await ebconnector.fetch_schedule_measurements_records_large_interval(
                frt_code=frontier, since=since_month, until=until_month
            )
        self.assertEqual(schedules, mocked_schedules)

    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_part_schedule_records(self, mock_get_auth_token):
        today = dt.datetime.now()
        since_month = today - dt.timedelta(weeks=WEEKS_TO_TEST)
        until_month = today
        since = until_month - dt.timedelta(weeks=WEEKS_TO_TEST - 2)
        ...
        for schedule in schedules:
            self.assertIn(schedule, mocked_schedules, "The schedule is not in mocked usages list")

    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_get_empty_schedule_records(self, mock_get_auth_token):
        today = dt.datetime.now()
        since_month = today - dt.timedelta(weeks=WEEKS_TO_TEST)
        until_month = today
        since = since_month - dt.timedelta(weeks=WEEKS_TO_TEST - 2)
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_parameter_validation_both_parameters(self, mock_get_auth_token):
        ...
        with self.assertRaises(InvalidParameterError) as context:
            await ebconnector.fetch_schedule_usage_records_large_interval(
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_parameter_validation_no_parameters(self, mock_get_auth_token):
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_parameter_validation_empty_strings(self, mock_get_auth_token):
    ...
    @patch("enerbitdso.enerbit.get_auth_token", new_callable=AsyncMock)
    async def test_meter_serial_filtering(self, mock_get_auth_token):
