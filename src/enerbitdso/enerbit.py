...
def _validate_query_parameters(frt_code: Optional[str], meter_serial: Optional[str]) -> None:
    ...
    if not has_frt and not has_meter:
        raise InvalidParameterError(
            "Debe especificar al menos uno de 'frt_code' o 'meter_serial'. No se pueden omitir ambos parámetros."
        )
...
async def get_auth_token(base_url: str, username: str, password: pydantic.SecretStr) -> dict:
    """Obtiene tokens de autenticación (access y refresh)"""
    path = "/auth/token/"
    data = {"username": username, "password": password.get_secret_value()}
    async with httpx.AsyncClient(base_url=base_url, timeout=TIMEOUT, verify=SSL_CONTEXT) as client:
...
async def get_client(base_url: str, username: str, password: pydantic.SecretStr) -> httpx.AsyncClient:
    url_parse: urllib.parse.ParseResult = urllib.parse.urlparse(base_url)
    url = url_parse.geturl()
    token_data = await get_auth_token(url, username, password)
    auth = {"Authorization": f"Bearer {token_data['access_token']}"}
    return httpx.AsyncClient(base_url=url, headers=auth, timeout=TIMEOUT, verify=SSL_CONTEXT)
...
def scale_measurement_records(records: list[ScheduleMeasurementRecord], scale: float):
    for r in records:
        ...
        r.reactive_energy_imported = (
            r.reactive_energy_imported * scale if r.reactive_energy_imported is not None else None
        )
        r.reactive_energy_exported = (
            r.reactive_energy_exported * scale if r.reactive_energy_exported is not None else None
        )
    return records
...
async def get_schedule_usage_records(
...
async def get_schedule_measurement_records(
    client: httpx.AsyncClient,
    since: dt.datetime,
    until: dt.datetime,
    meter_serial: Optional[str] = None,
    ...
    measurement_records = scale_measurement_records(measurement_records, scale=WATT_HOUR_TO_KILOWATT_HOUR)
...
class DSOClient:
    ...
    async def _refresh_access_token(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(base_url=self.api_base_url, timeout=TIMEOUT, verify=SSL_CONTEXT) as client:
                ...
                self._token_expires_at = (
                    _get_token_expiration(self._access_token) if self._access_token is not None else None
                )
    ...
    async def _authenticate(self) -> None:
        ...
        try:
            token_data = await get_auth_token(self.api_base_url, self.api_username, self.api_password)
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        async with self._lock:
            if not self._is_token_valid():
                if self._refresh_token and not await self._refresh_access_token():
            ...
            if self._client is None:
                ...
                self._client = httpx.AsyncClient(base_url=url, headers=auth, timeout=TIMEOUT, verify=SSL_CONTEXT)
                logger.debug("Created new HTTP client")

        return self._client

    async def _handle_auth_error(self, error: httpx.HTTPStatusError) -> None:
    ...
    async def fetch_schedule_usage_records_large_interval(
        self,
        since: dt.datetime,
        until: dt.datetime,
        meter_serial: Optional[str] = None,
        ...
        for i in range(0, number_of_requests):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    this_usage_records = await get_schedule_usage_records(
    ...
    async def fetch_schedule_measurements_records_large_interval(
        self,
        since: dt.datetime,
        until: dt.datetime,
        meter_serial: Optional[str] = None,
        ...
        for i in range(0, number_of_requests):
            ...
            for attempt in range(max_retries):
                try:
                    client = await self._get_client()
                    this_schedule_records = await get_schedule_measurement_records(
                        client, frt_code=frt_code, since=fi, until=ff, meter_serial=meter_serial
                    )
